...
Example:
    python3 compare_multiple_runs_from_s3.py \\
        s3://your-s3-bucket/jmeter-results/.../run_id=20251119-071502/ \\
        s3://your-s3-bucket/jmeter-results/.../run_id=20251119-072441/ \\
        s3://your-s3-bucket/jmeter-results/.../run_id=20251119-073304/ \\
...
class S3RunFetcher:
    ...
    def fetch_json_from_s3(self, s3_path: str, filename: str) -> Optional[Dict]:
        ...
        try:
            ...
            print(
                f"Warning: Could not fetch {filename} from {s3_path}: {e}",
                file=sys.stderr,
            )
            return None
    ...
    def fetch_run_data(self, s3_path: str) -> Dict:
        ...
        test_result = self.fetch_json_from_s3(
            s3_path, f"test_result_{path_metadata['run_id']}.json"
        )
        if not test_result:
        ...
        statistics = self.fetch_json_from_s3(
            s3_path, f"statistics_{path_metadata['run_id']}.json"
        )
        if not statistics:
        ...
        if test_result and "jmeter_run_summary" in test_result:
            jmeter_summary = self.parse_jmeter_summary(
                test_result["jmeter_run_summary"]
            )
...
class RunComparator:
    ...
    def extract_row_data(self, run: Dict) -> List:
        ...
        total_queries = perf_metrics.get(
            "total_queries", stats.get("Total", {}).get("sampleCount", 0)
        )
        ...
        # Get total duration
        total_duration = jmeter.get(
            "total_duration_sec", perf_metrics.get("actual_test_duration_sec", 0)
        )
    ...
    def print_summary(self):
        ...
        for run in self.runs:
            ...
            print(
                f"{run_id:<20} {instance:<20} {avg_lat if avg_lat else 'N/A':<12} {p99_lat if p99_lat else 'N/A':<12} {qps if qps else 'N/A':<10}"
            )
