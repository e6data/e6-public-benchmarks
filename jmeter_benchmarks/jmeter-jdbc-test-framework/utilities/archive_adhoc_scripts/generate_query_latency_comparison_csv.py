        ...
        if "run_type=" in line:
            ...
            if match:
                ...
                if test_results:
                    ...
                    concurrency_files[concurrency] = f"s3://your-s3-bucket/{latest}"
    ...
    try:
        ...
        for query_name, metrics in stats.items():
            ...
            query_data[query_name] = {
                ...
                "p90_sec": metrics.get("pct1ResTime", 0)
                / 1000.0,  # pct1 = 90th percentile
                "p95_sec": metrics.get("pct2ResTime", 0)
                / 1000.0,  # pct2 = 95th percentile
                "p99_sec": metrics.get("pct3ResTime", 0)
                / 1000.0,  # pct3 = 99th percentile
            }
    except Exception as e:
...
def generate_comparison_csv(
    ...
    print(
        f"Generating query latency comparison for {', '.join(engines)}...",
        file=sys.stderr,
    )
    ...
    for engine in engines:
        ...
        for concurrency in concurrency_levels:
            if concurrency not in test_results:
                print(
                    f"  Warning: Concurrency {concurrency} not found for {engine}",
                    file=sys.stderr,
                )
                continue
    ...
    with open(output_file, "w", newline="") as f:
        ...
        for query in all_queries:
            ...
            for engine in engines:
                for concurrency in concurrency_levels:
                    if (
                        concurrency in all_data[engine]
                        and query in all_data[engine][concurrency]
                    ):
            ...
            for engine in engines:
                for concurrency in concurrency_levels:
                    if concurrency in all_data[engine]:
                        values = [
                            q["avg_sec"] for q in all_data[engine][concurrency].values()
                        ]
                        if values:
    ...
    print(
        f"   Concurrency levels: {', '.join(map(str, concurrency_levels))}",
        file=sys.stderr,
    )
