...
def calculate_scaling_efficiency(
    baseline_perf: float, current_perf: float, baseline_conc: int, current_conc: int
) -> float:
    """
    ...
    efficiency = (
        expected_degradation / actual_degradation if actual_degradation > 0 else 0.0
    )
...
def generate_scaling_analysis(
    ...
    with open(output_file, "w") as f:
        ...
        f.write(
            f"| Concurrency | Avg Latency | p99 Latency | Degradation from C={concurrency_levels[0]} | Scaling Efficiency |\n"
        )
        f.write(
            f"|-------------|-------------|-------------|----------------------|--------------------|\n"
        )
        ...
        for conc in concurrency_levels:
            ...
            if conc == baseline_conc:
                ...
                efficiency = calculate_scaling_efficiency(
                    baseline_avg, m["avg"], baseline_conc, conc
                )
            ...
            f.write(
                f"| **C={conc}** | {m['avg']:.2f} sec | {m['p99']:.2f} sec | {degradation} | {efficiency_str} |\n"
            )
        ...
        f.write(
            f"- **Concurrency increase**: {conc_multiplier:.1f}x ({first_conc} → {last_conc})\n"
        )
        f.write(
            f"- **Latency increase**: {total_degradation:.1f}% ({first_avg:.2f}s → {last_avg:.2f}s)\n"
        )
        ...
        if total_degradation < 50:
            f.write(
                f"✅ **Production Ready**: {engine_name.upper()} shows good scaling behavior up to C={last_conc}.\n\n"
            )
        elif total_degradation < 150:
            f.write(
                f"⚠️ **Use with Caution**: Consider limiting production concurrency to C={concurrency_levels[len(concurrency_levels) // 2]} or below.\n\n"
            )
        else:
            f.write(
                f"🚨 **Not Recommended**: Severe performance degradation at high concurrency. Limit to C={concurrency_levels[1]} or lower.\n\n"
            )
        ...
        f.write(
            f"**Optimal Concurrency**: C={best_conc} (best throughput/latency ratio)\n"
        )
...
def main():
    ...
    print(
        f"Analyzing concurrency scaling for {engine_name.upper()} ({cluster_size})...\n"
    )
