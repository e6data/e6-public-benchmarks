...
Example:
    python test_queries_http.py connection_properties/my_http_connection.properties data_files/my_queries.csv
"""
...
def authenticate(props):
    ...
    auth_payload = {"user": props["USER"], "password": props["PASSWORD"]}
    ...
    try:
        response = requests.post(
            auth_url, json=auth_payload, headers=headers, timeout=30
        )
...
def execute_query(
    ...
    headers = {
        ...
        "Origin": (props or {}).get("origin_url", ""),
        "Referer": (props or {}).get("referer_url", ""),
        "Sec-Fetch-Mode": "cors",
...
def main():
    ...
    try:
        with open(query_file, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                queries.append({"alias": row["QUERY_ALIAS"], "query": row["QUERY"]})
    ...
    for i, query_obj in enumerate(queries, 1):
        ...
        result = execute_query(
            execute_url, token, alias, query, catalog, schema, cluster_name, props
        )
        ...
        if result["success"]:
            ...
            print(
                f"  ❌ FAILED - Status {result['status_code']} - {result['elapsed_ms']}ms"
            )
    ...
    for result in results:
        ...
        print(
            f"{status} {result['alias']}: Status {result['status_code']} - {result['elapsed_ms']}ms"
        )
    print()
        ...
        for result in failures:
            ...
            if isinstance(result.get("error"), dict):
                ...
                error_msg = result["error"].get(
                    "message", json.dumps(result["error"], indent=2)
                )
