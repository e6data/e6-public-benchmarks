...
def generate_multi_concurrency_csv(
    ...
    concurrency_data: Dict[
        int, Tuple[Dict, Dict, Dict]
    ],  # concurrency -> (stats1, stats2, mapping)
    output_file: Path,
):
...
def generate_multi_concurrency_summary(
    ...
    with open(output_file, "w") as f:
        ...
        f.write(
            f"| **Concurrency Levels** | {', '.join(map(str, concurrency_levels))} | {', '.join(map(str, concurrency_levels))} |\n\n"
        )
        ...
        for conc in concurrency_levels:
            ...
            f.write(
                f"| Metric | {engine1_name.upper()} | {engine2_name.upper()} | Difference |\n"
            )
            f.write(f"|--------|----------|-----------|------------|\n")
            ...
            f.write(
                f"| **Average** | {avg1:.2f} sec | {avg2:.2f} sec | {winner_icon} **{winner} {format_percentage(abs(avg_diff))} faster** |\n"
            )

            p99_winner_icon = "✅" if p99_diff > 0 else "⚠️"
            p99_winner = engine1_name.upper() if p99_diff > 0 else engine2_name.upper()
            f.write(
                f"| **p99** | {p99_1:.2f} sec | {p99_2:.2f} sec | {p99_winner_icon} **{p99_winner} {format_percentage(abs(p99_diff))} faster** |\n\n"
            )
        ...
        if total_wins_engine1 > total_wins_engine2:
            f.write(
                f"✅ **{engine1_name.upper()}** wins at {total_wins_engine1} out of {len(concurrency_levels)} concurrency levels.\n\n"
            )
            f.write(
                f"**Recommendation**: Use {engine1_name.upper()} for production workloads.\n\n"
            )
        elif total_wins_engine2 > total_wins_engine1:
            f.write(
                f"✅ **{engine2_name.upper()}** wins at {total_wins_engine2} out of {len(concurrency_levels)} concurrency levels.\n\n"
            )
            f.write(
                f"**Recommendation**: Use {engine2_name.upper()} for production workloads.\n\n"
            )
        else:
            f.write(
                f"📊 Both engines are competitive across different concurrency levels.\n\n"
            )
...
def main():
    ...
    if not common_concurrencies:
        print(
            "\nError: No matching concurrency levels found between the two engines.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for conc in sorted(common_concurrencies):
            ...
            # Create mapping
            mapping = create_query_mapping(
                stats1, stats2, parsed1.engine, parsed2.engine
            )
    ...
    generate_multi_concurrency_csv(
        parsed1.engine, parsed2.engine, concurrency_data, csv_file
    )
