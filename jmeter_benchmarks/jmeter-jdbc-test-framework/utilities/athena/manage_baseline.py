...
class BaselineManager:
    def __init__(self, region='us-east-1'):
        ...
        self.baseline_metadata_bucket = 'e6-jmeter'
    ...
    def mark_baseline(self, engine, cluster_size, benchmark, run_type, run_id, user, notes=''):
        ...
        print(f"✅ Found run: avg={data[1]}s, p50={data[2]}s, p90={data[3]}s, p95={data[4]}s, p99={data[5]}s")
        print()

        # Step 2: Create baseline metadata file in S3
        baseline_key = (f"{self.baseline_metadata_prefix}"
                       f"engine={engine}/"
    ...
    def _sync_to_athena(self, engine, cluster_size, benchmark, run_type):
        ...
        s3_path = (f"s3://e6-jmeter/jmeter-results/"
                  f"engine={engine}/"
                  f"cluster_size={cluster_size}/"
                  f"benchmark={benchmark}/"
                  f"run_type={run_type}/")
        ...
        try:
            ...
            print("   Run manually: python utilities/athena/upload_runs_index_to_athena.py --from-s3 " + s3_path)
            print()
    ...
    def show_baselines(self, engine=None):
        ...
        # Display baselines
        print(f"{'Engine':<12} {'Cluster':<12} {'Run Type':<18} {'Run ID':<20} {'Avg (s)':<10} {'Marked By':<15}")
        print("-" * 100)

        for b in baselines:
            print(f"{b['engine']:<12} {b['cluster_size']:<12} {b['run_type']:<18} "
                  f"{b['run_id']:<20} {b['metrics']['avg_latency_sec']:<10.2f} {b.get('marked_by', 'N/A'):<15}")

        print()

    def compare_with_baseline(self, engine, cluster_size, benchmark, run_type, run_id, source='s3', output_csv=None):
        """
        ...
        """
        if source == 'athena':
            return self._compare_with_baseline_from_athena(engine, cluster_size, benchmark, run_type, run_id, output_csv)
        else:
            return self._compare_with_baseline_from_s3(engine, cluster_size, benchmark, run_type, run_id, output_csv)

    def _compare_with_baseline_from_s3(self, engine, cluster_size, benchmark, run_type, run_id, output_csv=None):
        ...
        print("=" * 80)
        print()
        print(f"{'Metric':<15} {'Baseline':<15} {'Current':<15} {'Change':<15} {'Status':<10}")
        print("-" * 80)
        ...
        for metric_name, baseline_val, current_val in metrics_to_compare:
            ...
            if change_pct < -2:
                ...
                status = "➖ Same"
            print(f"{metric_name:<15} {baseline_val:<15.2f} {current_val:<15.2f} {change_pct:>+13.1f}% {status:<10}")
        ...
        if output_csv:
            self._write_comparison_csv(output_csv, engine, cluster_size, benchmark, run_type,
                                      baseline['run_id'], run_id, metrics_to_compare,
                                      overall_status, baseline.get('marked_by', 'unknown'))

    def _compare_with_baseline_from_athena(self, engine, cluster_size, benchmark, run_type, run_id, output_csv=None):
        ...
        if output_csv:
            self._write_comparison_csv(output_csv, engine, cluster_size, benchmark, run_type,
                                      baseline_metrics['run_id'], run_id, metrics_to_compare,
                                      overall_status, baseline_metrics.get('marked_by', 'unknown'))

    def _write_comparison_csv(self, csv_path, engine, cluster_size, benchmark, run_type,
                              baseline_run_id, current_run_id, metrics, overall_status, marked_by):
        ...
        with open(csv_path, 'a', newline='') as f:
            ...
            if not file_exists:
                writer.writerow([
                    'timestamp', 'engine', 'cluster_size', 'benchmark', 'run_type',
                    'baseline_run_id', 'current_run_id', 'baseline_marked_by',
                    'metric', 'baseline_value', 'current_value', 'change_pct', 'status',
                    'overall_status'
                ])
            ...
            for metric_name, baseline_val, current_val in metrics:
                ...
                writer.writerow([
                    timestamp, engine, cluster_size, benchmark, run_type,
                    baseline_run_id, current_run_id, marked_by,
                    metric_name, f"{baseline_val:.2f}", f"{current_val:.2f}",
                    f"{change_pct:.1f}", metric_status, overall_status
                ])
...
if __name__ == '__main__':
    ...
    compare_parser.add_argument('--source', choices=['s3', 'athena'], default='s3',
                                help='Data source: "s3" (baseline from S3, current from Athena) or "athena" (both from Athena)')
    compare_parser.add_argument('--output-csv', help='Optional CSV file path to append results for historical analysis')
    ...
    if args.command == 'mark':
        manager.mark_baseline(
            args.engine, args.cluster, args.benchmark,
            args.run_type, args.run_id, args.user, args.notes
        )
        ...
        manager.compare_with_baseline(
            args.engine, args.cluster, args.benchmark,
            args.run_type, args.run_id, args.source,
