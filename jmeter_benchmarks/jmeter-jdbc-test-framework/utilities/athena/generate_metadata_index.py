...
def extract_metadata_record(test_result: Dict, stats: Dict, s3_base_path: str, run_id: str,
                            engine: str, cluster_size: str, benchmark: str, run_type: str) -> Dict:
    """
    ...
    hold_period_sec = int(test_config.get('hold_period_min', 0)) if test_config.get('hold_period_min') else 0
...
def generate_metadata_index(s3_path: str, output_dir: str = '/tmp/metadata') -> bool:
    ...
    partition_dir = Path(output_dir) / f"engine={engine}" / f"cluster_size={cluster_size}"
    ...
    with open(output_file, 'w') as f:
        for i, run_id in enumerate(run_ids, 1):
            ...
            record = extract_metadata_record(
                test_result, stats, s3_path, run_id,
...
def main():
    ...
    print("     --result-configuration OutputLocation=s3://e6-jmeter/athena-query-results/")
    print("="*70)
