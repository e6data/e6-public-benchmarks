...
Usage:
    ...
    python utilities/generate_runs_index.py s3://your-s3-bucket/jmeter-results/engine=e6data/cluster_size=S-2x2/benchmark=tpcds_29_1tb/run_type=concurrency_8/

    # Generate indexes for all run_types under a benchmark
    python utilities/generate_runs_index.py s3://your-s3-bucket/jmeter-results/engine=e6data/cluster_size=S-2x2/benchmark=tpcds_29_1tb/ --all-run-types
...
def extract_run_metadata(
    ...
    actual_queries = len(
        [
            k
            for k in stats.keys()
            if k != "Total" and "BOOTSTRAP" not in k and "JSR" not in k
        ]
    )
    ...
    for query_name, query_stats in stats.items():
        if (
            query_name != "Total"
            and "BOOTSTRAP" not in query_name
            and "JSR" not in query_name
        ):
    ...
    return {
        ...
        "test_config": {
            ...
            "concurrent_threads": int(test_config.get("concurrent_threads", 0))
        ...
        "results_summary": {
            ...
            "excluded_queries": len(
                [k for k in stats.keys() if "BOOTSTRAP" in k or "JSR" in k]
            ),
            "total_success": total_stats.get("sampleCount", 0)
            - int(total_stats.get("errorCount", 0)),
...
def generate_runs_index(s3_path: str) -> Dict:
    ...
    for i, run_id in enumerate(run_ids, 1):
        ...
        # Extract metadata
        run_metadata = extract_run_metadata(
            test_result,
            stats,
            s3_path,
            run_id,
...
def main():
    ...
    parser.add_argument(
        "--upload", "-u", action="store_true", help="Upload generated index to S3"
    )
    ...
    print(
        f"Date Range: {index['index_info']['oldest_run']} → {index['index_info']['newest_run']}"
    )
    print("=" * 70)
